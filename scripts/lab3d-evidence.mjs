...
try{
 ...
 // Hold real File.text resolution to prove ownership, rather than relying on timing.
 await page.evaluate(()=>{const original=File.prototype.text;File.prototype.text=function(){if(this.name.startsWith('slow'))return new Promise(resolve=>{window.resolveLabRead=()=>original.call(this).then(resolve)});return original.call(this)}})
 await page.locator('#import').setInputFiles({name:'slow-a.json',mimeType:'application/json',buffer:Buffer.from(original)})
 await page.locator('#import').setInputFiles({name:'newer-b.json',mimeType:'application/json',buffer:Buffer.from(before)})
 await page.evaluate(()=>window.resolveLabRead());assert.equal(await canonical(),before)
 await page.locator('#import').setInputFiles({name:'slow-edit.json',mimeType:'application/json',buffer:Buffer.from(original)})
 await page.locator('#support').click();const edited=await canonical();await page.evaluate(()=>window.resolveLabRead());assert.equal(await canonical(),edited)
 await page.locator('#support').click();assert.equal(await canonical(),before)
