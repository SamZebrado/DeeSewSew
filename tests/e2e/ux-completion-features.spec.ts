...
for (const [name, width, height] of [['desktop', 1440, 1000], ['tablet', 768, 1024], ['phone', 390, 844]] as const) {
  test(`complete bilingual UI and guide at ${name}`, async ({ page }, info) => {
    ...
    await expect(page.locator('h1')).toHaveText('叠绣绣')
    await expect(page).toHaveTitle('叠绣绣 DeeSewSew — 数字刺绣工作室')
    await expect(page.locator('.eyebrow')).toHaveText('浏览器里的小小数字刺绣工作室。')
    await expect(page.locator('#leaf-guide')).toHaveText('绣一朵小花')
    ...
    await expect(page.locator('h1')).toHaveText('DeeSewSew')
    await expect(page).toHaveTitle('DeeSewSew — Tiny browser embroidery studio')
    await expect(page.locator('#guide-copy')).toHaveText('Flower step 4 of 41')
    ...
    await expect(page.locator('html')).toHaveAttribute('lang', 'en')
    await expect(page.locator('#guide-copy')).toHaveText('Flower step 4 of 41')
    await page.locator('#undo').click(); await expect(page.locator('#guide-copy')).toHaveText('Flower step 3 of 41')
    await page.locator('#redo').click(); await expect(page.locator('#guide-copy')).toHaveText('Flower step 4 of 41')
