...
describe('hoop view controller', () => {
  it('starts aligned to a stitchable front and snaps deterministically between faces', () => {
    ...
    view.snapBack()
    expect(view.snapshot()).toMatchObject({ yawDeg: 180, alignedFace: 'back', interactionState: 'editable', stitchable: true })
    view.snapFront()
  ...
  it('supports bounded keyboard increments and front/back shortcuts', () => {
    ...
    expect(view.snapshot()).toMatchObject({ pitchDeg: MAX_PITCH_DEGREES, face: 'front', alignedFace: null, interactionState: 'editable', stitchable: true })
