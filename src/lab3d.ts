...
function draw() {
  ...
  const age=releaseAt===null?1200:performance.now()-releaseAt
...
draw()
if ('serviceWorker' in navigator && import.meta.env.PROD) window.addEventListener('load',()=>{void navigator.serviceWorker.register(`${import.meta.env.BASE_URL}sw.js`,{scope:import.meta.env.BASE_URL}).catch(()=>{status.textContent='Offline setup unavailable; local lab editing remains available.'})})
