...
export const messages: Record<string, string> = {
  '3D Lab — Experimental':'3D 实验室 — 实验性功能',
  'Back to embroidery':'返回刺绣',
  'Desktop-first sphere study. Surface-laid thread, not through-fabric stitching.':'桌面优先的球面实验：线铺在表面，不模拟穿布刺绣。',
  'Lab controls':'实验室工具', 'View and support':'视角与支撑', 'History':'历史操作', 'Lab files':'实验文件',
  'Thread tool':'放线工具', 'Remove support':'移除支撑', 'Reinstall support':'恢复支撑',
  'Save lab':'保存实验', 'Load lab':'载入实验', 'Export lab JSON':'导出实验 JSON', 'Import lab':'导入实验',
  'Thread mode':'放线模式', 'Camera mode':'查看模式', 'anchors':'锚点', 'Support installed':'支撑已安装', 'Support removed':'支撑已移除',
  'Drag to orbit; wheel to zoom. In Thread mode, click the sphere to place an anchor. Focus the canvas: arrows orbit, +/- zoom, Home resets view, Enter places the center target, Escape cancels.':'拖动旋转，滚轮缩放。放线模式下点击球面添加锚点。聚焦画布后：方向键旋转，+/- 缩放，Home 重置视角，Enter 在中心准星处放线，Escape 取消。',
  'Experimental sphere and spatial thread':'实验球体与空间绣线',
  'Manual save: one separate experimental slot on this device. Save before leaving; export JSON for a portable backup. Ordinary artwork is unchanged.':'手动保存：本机提供一个独立实验存档位。离开前请保存；导出 JSON 可备份迁移。普通刺绣作品不受影响。',
  'Artistic rest-shape display, not real silk equilibrium. One sphere, one surface-contact run, at most 32 anchors. Visual desktop experiment; not full nonvisual or mobile 3D authoring.':'采用艺术性静止造型展示，不代表真实丝线的静力平衡。一个球体、一根表面接触线，最多 32 个锚点。这是桌面视觉实验，不提供完整的非视觉或移动端 3D 创作能力。',
  'This lab supports at most 32 anchors.':'本实验最多支持 32 个锚点。',
  'Choose a distinct point, not the opposite pole.':'请选择不同的点，且不要选择正对面的极点。',
  'Reinstall the support before placing an anchor.':'请先恢复支撑，再添加锚点。',
  'Local storage is full. Export a JSON backup.':'本机存储已满，请导出 JSON 备份。',
  'Local save is unavailable. Export a JSON backup.':'本机保存不可用，请导出 JSON 备份。',
  'Lab files must be at most 16 KB.':'实验文件不得超过 16 KB。',
  'Choose a valid experimental Lab JSON file. Your current work is unchanged.':'请选择有效的实验室 JSON 文件，当前作品未改变。',
  'Could not load the experimental slot. Your current work is unchanged.':'无法载入实验存档，当前作品未改变。',
  'Could not export the lab. Your current work is unchanged.':'无法导出实验，当前作品未改变。',
  'Could not place an anchor. Your current work is unchanged.':'无法添加锚点，当前作品未改变。',
  'Choose a point on the sphere.':'请选择球面上的点。', 'Anchor placed.':'已添加锚点。',
  'Thread mode: choose the center target or click the sphere.':'放线模式：选择中心准星或点击球面。',
  'Camera mode: view changes never add anchors.':'查看模式：调整视角不会添加锚点。',
  'Support removed; rest shape preserved.':'已移除支撑，保留静止造型。', 'Support reinstalled.':'已恢复支撑。',
  'Experimental lab saved locally.':'实验已保存到本机。', 'No experimental lab save yet.':'尚无实验存档。',
  'Experimental lab loaded.':'已载入实验。', 'Lab JSON download started.':'已开始下载实验 JSON。',
  'Experimental lab imported.':'已导入实验。', 'Choose Thread mode before placing an anchor.':'请先选择放线模式，再添加锚点。',
  'Pointer action cancelled.':'已取消指针操作。',
  'Offline setup unavailable; local editing remains available.':'离线准备不可用，仍可在当前页面编辑。',
